...
class Entity:
    ...
    def __add__(self: Self, other: Attribute | Order | LinkEntity) -> Self:
        if isinstance(other, Attribute):
            self.attributes.append(other)
            return self
        elif isinstance(other, Order):
            ...
            self.link_entities.append(other)
            # Return self so that the final expression is still an Entity
            return self
        raise TypeError(f"Cannot add Entity with {type(other)}")
...
entity = (
    ...
    + Attribute("incidentid")
    + Order("createdon", False)
    + (LinkEntity("contact") + Attribute("fullname") + Order("fullname", False))
)
